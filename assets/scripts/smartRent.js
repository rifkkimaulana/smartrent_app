document.addEventListener("DOMContentLoaded", () => {
  ...
  var app_name = "Appkit"; //Local Storage Names for PWA
  ...
  function init_template() {
    ...
    function selectHighlight() {
      var rememberHighlight = localStorage.getItem(app_name + "-Highlight");
    ...
    function highlightColors() {
      ...
      highlightData.forEach((el) =>
        el.addEventListener("click", (e) => {
          ...
          localStorage.setItem(app_name + "-Highlight", highlight);
        })
      );
      var rememberHighlight = localStorage.getItem(app_name + "-Highlight");
      if (rememberHighlight) {
    ...
    gradientData.forEach((el) =>
      el.addEventListener("click", (e) => {
        ...
        localStorage.setItem(app_name + "-Gradient", gradient);
      })
    );

    //Set Background and Highlight
    var pageBackground = localStorage.getItem(app_name + "-Gradient");
    if (pageBackground) {
    ...
    function checkDarkMode() {
      ...
      function activateDarkMode() {
        ...
        localStorage.setItem(app_name + "-Theme", "dark-mode");
      }
      function activateLightMode() {
        ...
        localStorage.setItem(app_name + "-Theme", "light-mode");
      }
      ...
      if (localStorage.getItem(app_name + "-Theme") == "dark-mode") {
        for (let i = 0; i < toggleDark.length; i++) {
          toggleDark[i].checked = "checked";
        }
        document.body.className = "theme-dark";
      }
      if (localStorage.getItem(app_name + "-Theme") == "light-mode") {
    ...
    if (checkVisited.length) {
      function check_visited_links() {
        var visited_links = JSON.parse(localStorage.getItem(app_name + "_Visited_Links")) || [];
        ...
        for (let i = 0; i < links.length; i++) {
          ...
          that.addEventListener("click", function (e) {
            ...
            if (visited_links.indexOf(clicked_url) == -1) {
              ...
              localStorage.setItem(app_name + "_Visited_Links", JSON.stringify(visited_links));
            }
          });
    ...
    if (visitDetection) {
      ...
      var lastVisitValue = localStorage.getItem(app_name + "-Last-Visited");
      var d = new Date();
      ...
      if (!lastVisitValue) {
        ...
        localStorage.setItem(app_name + "-Last-Visited", "Your last visit was " + strDate + " at " + strTime);
      } else {
    ...
    if (isPWA === true) {
      ...
      if (!checkPWA.classList.contains("isPWA")) {
        ...
        var now = Date.now();
        var setupTime = localStorage.getItem(app_name + "-PWA-Timeout-Value");
        if (setupTime == null) {
          localStorage.setItem(app_name + "-PWA-Timeout-Value", now);
        } else if (now - setupTime > hours * 60 * 60 * 1000) {
          localStorage.removeItem(app_name + "-PWA-Prompt");
          localStorage.setItem(app_name + "-PWA-Timeout-Value", now);
        }
        ...
        pwaClose.forEach((el) =>
          el.addEventListener("click", (e) => {
            ...
            localStorage.setItem(app_name + "-PWA-Timeout-Value", now);
            localStorage.setItem(app_name + "-PWA-Prompt", "install-rejected");
        ...
        if (pwaWindows.length) {
          if (isMobile.Android()) {
            if (localStorage.getItem(app_name + "-PWA-Prompt") != "install-rejected") {
